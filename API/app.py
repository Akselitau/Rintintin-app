...
from chalicelib.src.errors import CustomError, BadRequestError, InternalServerError, UnauthorizedError, NotFoundError
...
def handle_errors(event, get_response):
    try:
        ...
        response = Response(
            body={"Code": e.__class__.__name__, "Message": e.message},
            status_code=e.status_code,
            headers={"Content-Type": "application/json"}
        )
    except Exception as e:
        print(f"Unhandled exception: {e}")  # Log the unhandled exception
        response = Response(
