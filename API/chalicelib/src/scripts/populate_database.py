...
# -- Insert dogs
# INSERT INTO dogs (user_id, name, breed, profile_photo_url, information) VALUES
#     ((SELECT user_id FROM users WHERE email = 'alice@example.com'), 'Rex', 'Labrador', 'https://placedog.net/500/280?id=1', 'A good boi'),
#     ((SELECT user_id FROM users WHERE email = 'bob@example.com'), 'Bella', 'Beagle', 'https://placedog.net/500/280?id=2', 'Crazy as hell'),
#     ((SELECT user_id FROM users WHERE email = 'charlie@example.com'), 'Max', 'Bulldog', 'https://placedog.net/500/280?id=3', 'Doesn''t speak english')
# ON CONFLICT DO NOTHING;

# -- Insert pensions
# INSERT INTO pensions (user_id, name, address, phone, email, max_capacity, current_occupancy, rating, description, image_urls, equipment, hours, night_price) VALUES 
#     ((SELECT user_id FROM users WHERE email = 'alice@example.com'), 'Happy Paws', '10 Rue de Rivoli, 75001 Paris, France', '123-456-7890', 'info@happypaws.com', 20, 5, 4.5, 'A great place for your dog to stay.', 
#     '{"https://placedog.net/500/280?id=1", "https://placedog.net/500/280?id=4"}', '{"Wifi", "Air conditioning"}', '9am - 5pm', 23),
#     ((SELECT user_id FROM users WHERE email = 'bob@example.com'), 'Dog Haven', '20 Avenue de l''Opéra, 75001 Paris, France', '987-654-3210', 'contact@doghaven.com', 15, 3, 4.2, 'Safe and loving environment for your pet.', 
#     '{"https://placedog.net/500/280?id=2", "https://placedog.net/500/280?id=5"}', '{"Playground", "Swimming pool"}', '8am - 6pm', 30),
#     ((SELECT user_id FROM users WHERE email = 'charlie@example.com'), 'Puppy Paradise', '30 Boulevard de Sébastopol, 75004 Paris, France', '456-789-0123', 'info@puppyparadise.com', 25, 10, 4.8, 'The ultimate paradise for your puppy.', 
#     '{"https://placedog.net/500/280?id=3", "https://placedog.net/500/280?id=6"}', '{"Grooming", "Vet on call"}', '7am - 7pm', 20)
# ON CONFLICT DO NOTHING;

# -- Insert staff
# INSERT INTO staff (pension_id, certification_id, first_name, last_name, role, email, phone, image_url) VALUES 
#     ((SELECT pension_id FROM pensions WHERE name = 'Happy Paws'), 'CERT001', 'John', 'Doe', 'Manager', 'john.doe@example.com', '123-456-7890', 'https://randomuser.me/api/portraits/men/1.jpg'),
#     ((SELECT pension_id FROM pensions WHERE name = 'Dog Haven'), 'CERT002', 'Jane', 'Smith', 'Groomer', 'jane.smith@example.com', '098-765-4321', 'https://randomuser.me/api/portraits/women/2.jpg'),
#     ((SELECT pension_id FROM pensions WHERE name = 'Puppy Paradise'), 'CERT003', 'Emily', 'Jones', 'Trainer', 'emily.jones@example.com', '567-890-1234', 'https://randomuser.me/api/portraits/women/3.jpg')
# ON CONFLICT DO NOTHING;

# -- Insert reviews
# INSERT INTO reviews (pension_id, name, date, rating, comment) VALUES 
#     ((SELECT pension_id FROM pensions WHERE name = 'Happy Paws'), 'Alice', '2024-05-10', 4.5, 'Great place for my dog.'),
#     ((SELECT pension_id FROM pensions WHERE name = 'Dog Haven'), 'Bob', '2024-06-15', 4.2, 'Nice and clean.'),
#     ((SELECT pension_id FROM pensions WHERE name = 'Puppy Paradise'), 'Charlie', '2024-07-10', 4.8, 'My dog loved it!'),
#     ((SELECT pension_id FROM pensions WHERE name = 'Happy Paws'), 'David', '2024-08-10', 4.6, 'Very friendly staff.'),
#     ((SELECT pension_id FROM pensions WHERE name = 'Dog Haven'), 'Eve', '2024-09-15', 4.3, 'Good service.'),
#     ((SELECT pension_id FROM pensions WHERE name = 'Puppy Paradise'), 'Frank', '2024-10-10', 4.9, 'Highly recommend.'),
#     ((SELECT pension_id FROM pensions WHERE name = 'Happy Paws'), 'Grace', '2024-11-10', 4.4, 'Will come back again.'),
#     ((SELECT pension_id FROM pensions WHERE name = 'Dog Haven'), 'Hank', '2024-12-15', 4.6, 'Great experience.'),
#     ((SELECT pension_id FROM pensions WHERE name = 'Puppy Paradise'), 'Ivy', '2025-01-01', 4.7, 'Excellent care for my dog.'),
#     ((SELECT pension_id FROM pensions WHERE name = 'Happy Paws'), 'Jack', '2025-01-15', 4.5, 'Very satisfied.')
# ON CONFLICT DO NOTHING;

# -- Insert daily occupancy
# INSERT INTO daily_occupancy (pension_id, date, occupancy)
# SELECT p.pension_id, d, 0
# FROM pensions p,
